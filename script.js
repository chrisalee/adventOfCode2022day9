...
// convert coordinates to a single number so that we can compare if they are equal
...
const followPoint = (leadingPoint, followingPoint) => {
  ...
  const direction = pointDiff.map((component) =>
    component ? Math.sign(component) * (Math.abs(component) - 1) : 0
  );
...
// follow instructions and count the unique locations of the last tail knot
const countTailPositions = (instructions, tailLength) => {
  const { tailSet } = instructions.reduce(
    ({ tailSet, headPoint, tailPoints }, instructions) => {
      const [dir, steps] = instructions.split(' ');
...
// PART 2
const countOf9TailKnot = countTailPositions(instructions, 9);
console.log(countOf9TailKnot);
